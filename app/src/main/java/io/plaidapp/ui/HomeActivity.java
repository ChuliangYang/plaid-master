...
public class HomeActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gridImage.setLayoutManager(layoutManager);
        gridImage.addOnScrollListener(toolbarElevation);
        gridImage.addOnScrollListener(new InfiniteScrollListener(layoutManager, dataManager) {
            @Override
        ...
        gridImage.addItemDecoration(new GridItemDividerDecoration(this, R.dimen.divider_height,
                R.color.divider));
        gridImage.setItemAnimator(new HomeGridItemAnimator());

        RecyclerViewPreloader<Shot> shotPreloader =
                new RecyclerViewPreloader<>(this, adapter, shotPreloadSizeProvider, 4);
        gridImage.addOnScrollListener(shotPreloader);
        ...
        drawer.setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener() {
            ...
            public WindowInsets onApplyWindowInsets(View v, WindowInsets insets) {
                ...
                // inset the gridImage top by statusbar+toolbar & the bottom by the navbar (don't clip)
                gridImage.setPadding(
                        gridImage.getPaddingLeft() + insets.getSystemWindowInsetLeft(), // landscape
                        insets.getSystemWindowInsetTop()
                                + ViewUtils.getActionBarSize(HomeActivity.this),
                        gridImage.getPaddingRight() + insets.getSystemWindowInsetRight(), // landscape
                        gridImage.getPaddingBottom() + insets.getSystemWindowInsetBottom());
...
//        filtersList.setItemAnimator(new FilterAdapter.FilterAnimator());
    ...
    public void onActivityReenter(int resultCode, Intent data) {
        ...
        if (sharedShotId != -1L                                             // returning from a shot
                && adapter.getDataItemCount() > 0                           // gridImage populated
                && gridImage.findViewHolderForItemId(sharedShotId) == null) {    // view not attached
            ...
            gridImage.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
                @Override
                public void onLayoutChange(View v, int l, int t, int r, int b,
                                           int oL, int oT, int oR, int oB) {
                    gridImage.removeOnLayoutChangeListener(this);
                    startPostponedEnterTransition();
                }
            });
            gridImage.scrollToPosition(position);
    ...
    void checkEmptyState() {
        if (adapter.getDataItemCount() == 0) {
            // if gridImage is empty check whether we're loading or if no filters are selected
